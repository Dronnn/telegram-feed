...
- Отложенные older-preview карточки из “тихих” каналов не должны влетать в ленту отдельным недельным слоем раньше общей истории; они должны смешиваться с остальными older-batches в одном общем хронологическом порядке.
- После верхней догрузки старых карточек экран должен оставаться на том же верхнем сообщении; добавленный сверху older-batch не должен сам по себе сдвигать viewport на часы, дни или недели назад.
...
- Дедупликация Telegram-сообщений должна везде использовать составной ключ `chatId + messageId`, а не голый `message.id`, потому что message IDs уникальны только внутри конкретного чата.
